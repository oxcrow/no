...
let posOfLoc loc =
  match loc with Ast.Location l -> (l.lineIndex, l.colIndex) | Ast.Nowhere -> never source "wut?"
;;
...
let scream file lineIndex colIndex =
  ...
  let message lineIndex =
    ...
    | true -> (fmt "//%6d │ " lineIndex, List.nth lines (lineIndex - 1))
    | false -> (fmt "//     ~ │", "")
  in
...
let what message file loc =
  let header = errorLine message in
  let lineIndex, colIndex = posOfLoc loc in
  let code = scream file lineIndex colIndex in
  fmt "%s\n%s" header code
;;

let quack source message file loc =
  raise (Report { message = what message file loc; source = xSOURCE source; error = None })
;;

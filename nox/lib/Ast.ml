...
(* Statements *)
and stmts =
  | LetStmt of { vars : vars list; pats : pats list; expr : exprs; stmtId : int; loc : loc }
...
let rec getTypeOfExprOpt e =
  match e with
  | TupleExpr e -> Some e.types
  | ArrayExpr e -> Some e.types
  | DerefExpr e -> (
      match getTypeOfExprOpt e.expr with Some (ConRefType t) -> Some t.types | _ -> None)
  | ConRefExpr e -> Some e.types
  | IfExpr e -> Some e.types

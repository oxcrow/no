...
stmts:
    | LET p=nonempty_seplist(COMMA,pats); EQ e=exprs; SEMICOLON {
        Ast.LetStmt {expr=e; vars=[]; pats=p; stmtId=(stmtId()); loc=(loc $loc)}
    }
...
lvalPostExprs:
    ...
    | e=lvalExprs; DOT STAR %prec DEREF {
...
vars:
    | s=states; n=names; t=option(types); {
        Ast.Var {state=s; name=n; exprId=None; type'=(match t with Some t -> t | None -> Ast.NoneType); uuid=(uuid ()); loc=(loc $loc)}
    }

args:
    | n=names; t=types; {
        Ast.Var {state=Ast.ConState; exprId=None; name=n; type'=t; uuid=(uuid ()); loc=(loc $loc)}
    }
